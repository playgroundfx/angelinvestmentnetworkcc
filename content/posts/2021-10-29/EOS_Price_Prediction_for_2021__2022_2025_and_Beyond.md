...
## Weekly Elliott wave EOS analysis as of 29.10.2021

The EOSUSD price chart displays the structure of the middle leg of the
global double zigzag (W)-(X)-(Y), where sub-wave (W) has completed.
There is unfolding the linking down wave (X) as a double combination
composed of sub-waves W-X-Y, which also have complex structures. There
should be forming the initial part of the final sub-wave Y, the
structure of which is schematically marked by sub-waves [W]-[X]-[Y]. Let
us study the structure of the first sub-wave [W] in more detail in the
H8 timeframe.

The [W] wave is unfolding as a simple zigzag, with the bearish impulse
wave (A) completed inside. There is now unfolding the horizontal
corrective wave (B) as a contracting triangle A-B-C-D-E. There is now
forming the middle leg of the triangle, correction C, with sub-waves [A]
and [B] completed inside. Therefore, the price should be rising in the
[C] impulse to a level of 5.53, which serves as resistance. An
approximate trajectory of the EOS future price movement is outlined in
the chart.
